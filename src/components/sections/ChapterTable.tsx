                            ...
                            <TableHeader className="sticky top-0 dark:bg-gray-950 bg-gray-50 mx-4">
                                <TableRow>
                                    ...
                                    {!hiddenColumns.includes("update") && (
                                        <TableHead className="text-right truncate">Cập nhật</TableHead>
                                    )}
                                    {!hiddenColumns.includes("view") && (
                                        <TableHead className="text-right truncate">Lượt xem</TableHead>
                                    )}
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                ...
                                {chapters.map(({ id, title, createdAt, mobileOnly, type, view }) => {
                                    ...
                                    const href = `/${type === ContentType.movie ? "episode" : "chapter"}/${slug(
                                        slugTitle
                                    )}-${id}`;
    ...
    return (
        <TableRow ref={rowRef}>
            <TableCell className="font-medium font-mono max-w-md">
                <Link href={href} className={cn("flow-root max-w-full", isActive && "text-blue-500")}>
            ...
            {!hiddenColumns!.includes("update") && (
                <TableCell className="font-mono text-right">{numeral(view || 0).format("0.0a")}</TableCell>
            )}
        </TableRow>
    );
}
