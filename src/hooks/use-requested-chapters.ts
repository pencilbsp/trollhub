...
import { getRequestedChapters } from '@/actions/guest/chapter-actions';
...
export default function useRequestedChapters({ start = 0, end = 12, type, sort }: Props) {
    ...
    const swrKey = useMemo(() => new URLSearchParams({ start: range.start.toString(), end: range.end.toString(), type, sort }), [range, type, sort]);
